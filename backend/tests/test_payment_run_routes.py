...
_EXPLAIN_MOCK_RESPONSE = {
    "suggestion_id": "placeholder",
    "display_name": "iCloud+ 2TB",
    "service_guess": "Apple iCloud+ storage subscription",
    "amount_mode_rationale": "Fixed $10.09 monthly charge consistent across 12 occurrences.",
    "rule_hints": {
    ...
    "confidence_note": "High confidence recurring pattern.",
}


def test_explain_200_mocked(monkeypatch, client, data_dir, payment_worksheet_env):
    import routes.payment_run as payment_run_routes
    ...
    async def fake_fetch(_client, _http, *, api_key, suggestion_id, lookback_months=12, model=None):
        return {**_EXPLAIN_MOCK_RESPONSE, "suggestion_id": suggestion_id}

    monkeypatch.setattr(payment_run_routes, "fetch_bill_suggestion_explain", fake_fetch)
    try:
        list_response = client.get("/api/payment-run/bill-suggestions")
        assert list_response.status_code == 200
        suggestion_id = list_response.json()["data"][0]["id"]
        response = client.post(
            f"/api/payment-run/bill-suggestions/{suggestion_id}/explain",
            json={"lookback_months": 12},
        )
        assert response.status_code == 200
        body = response.json()
...
def test_explain_list_unchanged(monkeypatch, client, data_dir, payment_worksheet_env):
    import routes.payment_run as payment_run_routes
    ...
    monkeypatch.setattr(payment_run_routes, "fetch_bill_suggestion_explain", fake_fetch)
    try:
        before = client.get("/api/payment-run/bill-suggestions")
        assert before.status_code == 200
        before_body = before.json()
        before_ids = {row["id"] for row in before_body["data"]}
        before_count = len(before_body["data"])
        assert before_count > 0

        explain_target = next(
            (row["id"] for row in before_body["data"] if row.get("status") == "review"),
            before_body["data"][0]["id"],
        )
        explain_response = client.post(
            f"/api/payment-run/bill-suggestions/{explain_target}/explain",
            json={"lookback_months": 12},
        )
        assert explain_response.status_code == 200

        after = client.get("/api/payment-run/bill-suggestions")
        assert after.status_code == 200
        after_body = after.json()
        after_ids = {row["id"] for row in after_body["data"]}
        assert after_ids == before_ids
        assert len(after_body["data"]) == before_count
    finally:
        app.dependency_overrides.pop(get_firefly_client, None)


def test_explain_body_lookback(monkeypatch, client, data_dir, payment_worksheet_env):
    import routes.payment_run as payment_run_routes
    ...
    async def fake_fetch(_client, _http, *, api_key, suggestion_id, lookback_months=12, model=None):
        captured["lookback_months"] = lookback_months
        return {**_EXPLAIN_MOCK_RESPONSE, "suggestion_id": suggestion_id}
    ...
    try:
        ...
        response = client.post(
            f"/api/payment-run/bill-suggestions/{suggestion_id}/explain",
            json={"lookback_months": 6},
        )
        assert response.status_code == 200
        assert captured["lookback_months"] == 6
    finally:
        app.dependency_overrides.pop(get_firefly_client, None)
